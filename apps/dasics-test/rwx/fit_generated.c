...
int fit_init_static(void) {
    ...
    entry->bounds_data[0].lo = (uint64_t)&__ULIBTEXT_TEST_RWX_BEGIN__;
    entry->bounds_data[0].hi = (uint64_t)&__ULIBTEXT_TEST_RWX_END__;

    // 2. Data segment (read-write)
    entry->bounds_data[1].perm = DASICS_LIBCFG_R | DASICS_LIBCFG_W;
    entry->bounds_data[1].lo = (uint64_t)&__ULIBDATA_TEST_RWX_BEGIN__;
    entry->bounds_data[1].hi = (uint64_t)&__ULIBDATA_TEST_RWX_END__;

    // 3. Read-only data segment (read-only)
    entry->bounds_data[2].perm = DASICS_LIBCFG_R;
    entry->bounds_data[2].lo = (uint64_t)&__ULIBRODATA_TEST_RWX_BEGIN__;
    entry->bounds_data[2].hi = (uint64_t)&__ULIBRODATA_TEST_RWX_END__;

    // 4. BSS segment (read-write)
    entry->bounds_data[3].perm = DASICS_LIBCFG_R | DASICS_LIBCFG_W;
    entry->bounds_data[3].lo = (uint64_t)&__ULIBBSS_TEST_RWX_BEGIN__;
    entry->bounds_data[3].hi = (uint64_t)&__ULIBBSS_TEST_RWX_END__;

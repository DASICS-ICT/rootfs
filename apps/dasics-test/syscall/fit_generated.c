...
/* Stack frame bound: len = frame size in bytes; stored in mem_bounds with lo=UINT64_MAX */
...
int fit_init_static(void) {
    ...
    ADD_CODE_BOUND(entry, DASICS_LIBCFG_X, __ULIBTEXT_TEST_SYSCALL_BEGIN__, __ULIBTEXT_TEST_SYSCALL_END__);
    // 2. Data segment (read-write)
    ADD_MEM_BOUND(entry, DASICS_LIBCFG_R | DASICS_LIBCFG_W, __ULIBDATA_TEST_SYSCALL_BEGIN__, __ULIBDATA_TEST_SYSCALL_END__);
    // 3. Read-only data segment (read-only)
    ADD_MEM_BOUND(entry, DASICS_LIBCFG_R, __ULIBRODATA_TEST_SYSCALL_BEGIN__, __ULIBRODATA_TEST_SYSCALL_END__);
    // 4. BSS segment (read-write)
    ADD_MEM_BOUND(entry, DASICS_LIBCFG_R | DASICS_LIBCFG_W, __ULIBBSS_TEST_SYSCALL_BEGIN__, __ULIBBSS_TEST_SYSCALL_END__);
    // 5. Stack frame (read-write)
    ADD_STACK_BOUND(entry, 144);

...
static void print_usage(const char *prog) {
    printf("Usage: %s --reg <s0|s1|...|s11> --case <viol_write|viol_read|proto_mismatch|legal_save_restore|post_save_cipher_read|missing_restore_return|auth_tamper_bitflip|auth_tamper_overwrite>\n",
           prog);
}
...
int main(int argc, char **argv) {
    ...
    } else if (strcmp(case_name, "missing_restore_return") == 0) {
        rc = sreg_case_missing_restore_return(reg_index);
